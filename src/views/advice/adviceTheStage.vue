...
<div id="components-layout-demo-basic">
        ...
        <a-table :columns="columns" :dataSource="data" :pagination="pagination" @change="handleTableChange"  rowKey="id" key="index">
            ...
            <span slot="action" slot-scope="text, record">
             <!-- <a href="javascript:;" @click="bing(record)"></a>-->
              ...
              >
                <a class="delBtn"><a-icon type="delete" />关闭</a>
              </a-popconfirm>
    ...
    <div class="statusLi">
		<ul>
			<!--<li class="statusLi_1">待处理 >>></li>
			<li class="statusLi_2">处理中 >>></li>
			<li class="statusLi_3">待评价 >>></li>
			<li class="statusLi_4">已评价 >>></li>
			<li class="statusLi_5">已关闭</li>-->
			<li :class="status==='待处理'?'ccc':'aaa'">待处理 >>></li>
			<li :class="status==='处理中'?'ccc':'aaa'">处理中 >>></li>
			<li :class="status==='待评价'?'ccc':'aaa'">待评价 >>></li>
...
import { schemeList,schemeDel,getFenpeiList,downloadExcel,itemEdit,adviceListQ,adviceFile,adviceNewTimQ,adviceCloseQ,adviceFileQ,adviceAddQ,adviceStarQ } from '@/api/newpage'
const columns = [{
                  title: '标题',
                  dataIndex: 'title',
                  },  {
...
export default {
  ...
  methods: {
  	...
  	handleOk_HF(){
  		adviceAddQ(this.huifu).then(res => {
	        console.log(res)
	        if(res.msg == "success"){
	        	alert("操作成功")
	      ...
	      adviceFileQ(obj).then(res => {
	        ...
	        if(res.msg == "success"){
			        ...
			        console.log(this.status)
						...
						if(status == "待处理"){
							$('.statusLi_1').css('background','rgb(28, 239, 196)')
						}else if(status == "处理中"){
							sa.css('background','#18d8b1')
							sb.css('background','rgb(28, 239, 196)')
						}else if(status == "待评价"){
