    ...
    <a-layout>
      ...
      <a-layout-content>
       ...
       <!--<a class="daochu" @click="daochu" style="font-weight: bold;color: #ffffff;background: #0080FF;border-radius: 2px; padding: 1px 5px;margin-bottom:20px;margin-left: 20px;" >一键导出</a>-->
        ...
        <a-table :columns="columns" size="middle" :dataSource="data"  @change="handleTableChange" :pagination="false"  rowKey="id" key="index" :scroll="{ x: 1200, y: 800 }">
...
import { schemeList,schemeDel,getFenpeiList,downloadExcel,itemEdit,adviceList,adviceFile,jifenList,jifenGailan} from '@/api/newpage'
...
const columns = [{
                  ...
                  title: '阅读更正',
                  align: 'center',
                  dataIndex: 'readUpdate',
                  width:200
                  }, {
                  title: '查看更正',
                  align: 'center',
                  dataIndex: 'sameUpdate',
                  width:200
                  },{
                  title: '注册激活',
                  align: 'center',
...
export default {
  ...
  methods: {
   ...
   getFenpeiList() {
    ...
    schemeList() {
      jifenGailan(this.foo).then(res => {
        console.log(res)
        this.zongjifen = res.total
        this.data = res.list
       /* if (res.count) {
