<template>
  <a-card :bordered="false">
    ...
    <div class="table-page-search-wrapper">
      <a-form layout="inline">
        <a-row :gutter="12">
          ...
          <a-col :md="7" :sm="8">
            <a-form-item label="产品名称" :labelCol="{span: 6}" :wrapperCol="{span: 14, offset: 1}">
              <a-input v-model="queryParam.produceName"></a-input>
            </a-form-item>
          </a-col>
          <a-col :md="7" :sm="8">
    ...
    >
      <a-form :form="form">
        <a-form-item label="产品名称：" :label-col="{ span: 5 }" :wrapper-col="{ span: 18 }">
          <a-input v-model="produceName" />
        </a-form-item>
...
import { addProduce, deleteProduce, allprojectList, editProduce,deleteProduceBatch } from '@/api/api'
export default {
  name: 'DictList',
  ...
  methods: {
    ...
    handleEdit(record) {
      this.visible = true
      this.produceName = record.produceName
    ...
    handleOk() {
      ...
      let obj
      if (!this.id) {
        obj = addProduce(formData)
      } else {
        formData.id = this.id
        obj = editProduce(formData)
      }
      obj
        .then(res => {
          if (res.success) {
    ...
    deleteBtn() {
      ...
      this.$confirm({
        ...
        onOk() {
          ...
          let result = deleteProduceBatch(obj)
          result.then(res => {
            if (res.success) {
              that.$message.success(res.message)
              that.loadData()
