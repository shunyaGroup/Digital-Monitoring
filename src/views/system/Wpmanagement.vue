...
import { addProject, deleteProject, projectList, editProject, deleteProjectBatch } from '@/api/api'
export default {
  name: 'DictList',
  ...
  data() {
    return {
      ...
      url: {
        ...
        delete: '/project/aiProject/delete',
    ...
    deleteBtn() {
      ...
      this.$confirm({
        ...
        onOk() {
          ...
          let result = deleteProjectBatch(obj)
          result.then(res => {
            if (res.success) {
              that.$message.success(res.message)
              that.loadData()
